        ...
        if (mNextNativeState == NativeState.RESUMED) {
            if (mSurface.mIsSurfaceReady && (mHasFocus || mHasMultiWindow) && mIsResumedCalled) {
                if (mSDLThread == null) {
                    ...
                    // psxrecomp: the recompiled guest runs on this thread and
                    // expects a desktop-sized stack (the MinGW release links a
                    // 64 MB one); ART's ~1 MB default overflows. Java thread
                    // stack sizes are honoured by ART. [tomba2 SDL Java patch #1]
                    mSDLThread = new Thread(null, new SDLMain(), "SDLThread", 64L << 20);
